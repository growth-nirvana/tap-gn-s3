...
class CSVFileStream(Stream):
    ...
    def get_records(self, context: Optional[dict] = None) -> Iterable[Dict[str, Any]]:
        ...
        # We sort here so that tracking the modified_since bookmark makes sense
        for s3_file in sorted(s3_files, key=lambda item: item["last_modified"]):
            ...
            for row_number, row in enumerate(reader, start=1):
                ...
                # Convert row keys to BigQuery compliant names
                sanitized_row = {
                ...
                yield {
                    ...
                    "_last_modified": last_modified,  # Add the last_modified timestamp to each record
                    **sanitized_row,
                }
